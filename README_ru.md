...
Примеры файлов и описание импорта — в `examples/README_ru.md`.
...
4. Нажмите **Загрузить распакованное** и выберите корневую папку проекта (этот репозиторий).

---

## Разработка

Запуск браузера с заранее заполненным профилем для ручной проверки popup:

```bash
npm run dev:seeded
```

Команда создаёт изолированный временный профиль Chrome/Chromium, загружает временную копию распакованного расширения и записывает детерминированные прокси/правила в `chrome.storage.sync`. Если Chrome не найден автоматически, используйте `CHROME_PATH=/path/to/chrome npm run dev:seeded` или `npm run dev:seeded -- --browser /path/to/chrome`.
