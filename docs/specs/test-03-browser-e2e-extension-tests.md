...
- 2026-07-12: Marked task blocked because the only locally available browser is branded Google Chrome, which exposes built-in extension targets but does not load this repo as an unpacked extension through command-line flags in this environment. Unblock by installing or pointing `--browser`/`CHROME_PATH` to Chrome for Testing or Chromium and rerunning `npm run e2e:extension`.
