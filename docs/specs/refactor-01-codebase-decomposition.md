...
extension/popup.js (869 lines) and extension/background.js (679 lines) are monoliths. extension/popup.js lives
entirely inside a single `DOMContentLoaded` closure -- nothing is exported, nothing
is testable, 20+ variables share one scope. extension/background.js contains a 198-line god
...
- Split extension/popup.js and extension/background.js into focused ES modules by responsibility.
...
- Must: Decompose extension/popup.js into modules:
  - `extension/popup/popup.js` -- entry point, init, screen routing.
...
- Must: Decompose extension/background.js into modules:
  - `extension/background/background.js` -- entry point, listeners, message router.
...
- Must: Shared storage layer (`extension/shared/storage.js`) -- single storage access module imported by both popup and background.
- Must: Magic numbers extracted into constants -- timeouts, debounce, retry params in a `TIMEOUTS` object in extension/utils.js.
- Must: Remove dead code -- `tabs.js` reference in manifest.json, unused `notifications` permission.
- Must: Move inline styles from extension/popup.html into extension/popup.css.
- Should: Single `buildProxyOptions(proxies)` function instead of 3 duplicates in extension/popup.js.
- Should: Consistent async/await instead of mixed callbacks and promises in extension/background.js.
- Should: Extract form validation into standalone pure functions.
- Could: Optimize extension/icon48.png (1.5 MB is excessive for a 48px icon).
- Could: Add JSDoc comments to exported functions in new modules.

## Risks / questions

- Manifest V3 service worker loads as ES module. Moving extension/background.js to `extension/background/background.js` requires updating the path in manifest.json. Similarly extension/popup.html must reference the new `extension/popup/popup.js`.
...
1. Replace all raw storage key strings with `STORAGE_KEYS.*` in extension/popup.js.
2. Extract magic numbers into a `TIMEOUTS` object in extension/utils.js.
3. Remove dead code from manifest.json (`tabs.js` reference, `notifications` permission).
4. Move inline styles from extension/popup.html to extension/popup.css.
5. Verify: load extension, check main scenarios.

### Phase 2: Decompose extension/popup.js (~4-6 hours)
...
7. Keep `extension/popup/popup.js` as entry point: init, screen routing, wiring.
8. Update extension/popup.html: `<script src="popup/popup.js" type="module">`.
9. Verify: full walkthrough of all screens.

### Phase 3: Decompose extension/background.js (~3-4 hours)
...
5. Keep `extension/background/background.js` as entry point: listeners, message router.
6. Update manifest.json: `"service_worker": "extension/background/background.js"`.
7. Verify: proxy toggle, per-page, auth, tab close cleanup.

### Phase 4: Shared storage layer (~2-3 hours)

1. Create shared `extension/shared/storage.js` (or adapt popup/storage.js for use by both sides).
...
- When renaming files, update ALL references (manifest.json, extension/popup.html, imports, documentation).
...
- 2026-02-13: **Phase 1 completed.** Quick wins:
  - 1.1: Replaced all 10 raw storage key strings in extension/popup.js (`"proxies"`, `"siteRules"`, etc.) with `STORAGE_KEYS.*` constants, including both `get()` and `set()` calls. Also converted property access on settings objects to bracket notation (`settings[STORAGE_KEYS.x]`).
  - 1.2: Created `TIMEOUTS` object in extension/utils.js with 6 named constants (`proxyCheckDelay`, `debounceApply`, `tabRetryInterval`, `tabMaxAttempts`, `hintDuration`, `feedbackDuration`). Replaced all magic numbers in extension/popup.js (retry logic, hint/feedback durations) and extension/background.js (debounce 100ms, checkCurrentProxySettings 1000ms delays).
  ...
  - 1.4: Moved all inline `<style>` block (58 lines of logging toggle CSS) from extension/popup.html into extension/popup.css. Replaced inline `style="display: none"` on file inputs with `.hidden-file-input` CSS class.
- 2026-02-13: **Phase 2 completed.** Decomposed extension/popup.js (869 lines) into 6 focused modules under `extension/popup/`:
  ...
  - `extension/popup/popup.js` — entry point (~160 lines): screen routing, logging toggle, tab info retrieval, initialization. Wires modules via dependency injection.
  - Updated `extension/popup.html`: `<script>` src changed from `extension/popup.js` to `extension/popup/popup.js`.
  - Old root `extension/popup.js` is now superseded by `extension/popup/popup.js` (can be deleted after verification).
- 2026-02-13: **Phase 3 completed.** Decomposed extension/background.js (679 lines) into 4 modules under `extension/background/`:
  ...
  - `extension/background/background.js` — entry point (~190 lines): logger setup, tab reload helpers, debug functions, debounce, storage listener, message router, wires all modules.
  - Updated manifest.json: `service_worker` changed from extension/background.js to `extension/background/background.js`.
  - Fixed missed hardcoded `1000` (line 452 in old file) → `TIMEOUTS.proxyCheckDelay`.
  - Old root `extension/background.js` is now superseded by `extension/background/background.js` (can be deleted after verification).
- 2026-02-13: **Phase 4 completed.** Shared storage layer:
  - Added `extension/shared/storage.js` as the single chrome.storage.sync access module.
...
- 2026-02-13: **Phase 5 completed.** Final cleanup:
  - Added explicit WARNING comments in `extension/utils.js` to keep PAC template logic in sync.
  - Updated AGENTS.md to reflect new file structure and docs/specs path.
  - Manual verification completed (extension loaded and scenarios checked).
- 2026-02-13: **Repo layout updated.** Kept manifest.json at repo root; moved extension code under `extension/` and updated docs/workflows to match.
